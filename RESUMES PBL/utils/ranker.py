...
def rank_uploaded_resumes(parsed_list, required_skills, model, scaler, feature_cols):
    ...
    for parsed in parsed_list:
        ...
        cols_present = [c for c in SCALE_COLS if c in row_df.columns]
        row_df[cols_present] = scaler.transform(row_df[cols_present])

        # Predict
        ml_score   = float(model.predict(row_df[feature_cols])[0])
